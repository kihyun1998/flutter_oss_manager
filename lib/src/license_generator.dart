...
class LicenseGenerator {
  ...
  String _encodeLicenseText(String raw) {
    return base64.encode(gzip.encode(utf8.encode(raw)));
  }
  ...
  void _writeDartFile(String outputPath, List<OssLicense> ossLicenses) {
    ...
    buffer.writeln('// License text is stored gzip+base64 encoded to reduce');
    buffer.writeln('// resident memory. Decoded lazily via the licenseText');
    buffer.writeln('// getter. Uses dart:io; not compatible with Flutter Web.');
    buffer.writeln('');
    ...
    buffer.writeln('  final String _encodedLicenseText;');
    buffer.writeln('');
    ...
    buffer.writeln('    required String encodedLicenseText,');
    buffer.writeln('    this.repositoryUrl,');
    buffer.writeln('    this.description,');
    buffer.writeln('  }) : _encodedLicenseText = encodedLicenseText;');
    buffer.writeln('');
    buffer.writeln('  String get licenseText => utf8.decode(');
    buffer.writeln('      gzip.decode(base64.decode(_encodedLicenseText)));');
    buffer.writeln('}');
    buffer.writeln('');
    buffer.writeln('const List<OssLicense> ossLicenses = [');
    for (final license in ossLicenses) {
      final encoded = _encodeLicenseText(license.licenseText);
      ...
      buffer.writeln('    encodedLicenseText: ${jsonEncode(encoded)},');
      buffer.writeln('  ),');
    }
    buffer.writeln('];');
